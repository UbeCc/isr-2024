...
def cubic_time_scaling_theta(Tf, t, theta_start, theta_end):
    s = 3 * (1.0 * t / Tf) ** 2 - 2 * (1.0 * t / Tf) ** 3
    ds = 6 * (1.0 * t / Tf ** 2) - 6 * (1.0 * t ** 2 / Tf ** 3)
    dds = 6 * (1.0 / Tf ** 2) - 12 * (1.0 * t / Tf ** 3)
    theta_t = theta_start + s * (theta_end - theta_start)
    dtheta_t = ds * (theta_end - theta_start)
    ddtheat_t = dds * (theta_end - theta_start)
    return theta_t, dtheta_t, ddtheat_t
...
def cubic_time_scaling_theta2(Tf, dt, theta_start, theta_end):
    ...
    :return: esired_position, desired_velocity, desired_acceleration
    """
    ...
    while t <= Tf:
        ...
        dtheta_t = ds * (theta_end - theta_start)
        ddtheat_t = dds * (theta_end - theta_start)
        desired_position.append(theta_t)
        desired_velocity.append(dtheta_t)
        desired_acceleration.append(ddtheat_t)
        t += dt
...
class pandaEnv():
    ...
    def __init__(self, physicsClientId, base_position=(0.0, 0.0, 0.0), control_orientation=1,
        ...
        # x is randomly sampled from 0.35 to 0.45
        # y is randomly sampled from -0.35 to -0.25
        # x = np.random.uniform(0.35, 0.45)
        # y = np.random.uniform(-0.35, -0.25)
        # self._obj_init_pose = [x, y, 0.03, 0, 0, 0, 1]
        self.range = 0.05

        self.camera_width = 256

        self.obj_id = None
        # set np random seed to 10000
    ...
    def init(self):
        ...
        robot_file_path = os.path.join(os.getcwd(), "homework1/robot_model/franka_panda/panda_model.urdf")
    ...
    def panda_camera(self):
        # print the self.robot links
        # for i in range(20):
        #     print(i, p.getLinkState(self.robot_id, i, computeForwardKinematics=True))
        # raise NotImplementedError("Camera not implemented!")
        # Center of mass position and orientation (of link-7)
        com_p, com_o, _, _, _, _ = p.getLinkState(self.robot_id, 11, computeForwardKinematics=True)
        rot_matrix = p.getMatrixFromQuaternion(com_o)
        rot_matrix = np.array(rot_matrix).reshape(3, 3)
        # Initial vectors
        init_camera_vector = (1, 0, 1) # z-axis
        init_up_vector = (0, 1, 0) # y-axis
        # Rotated vectors
        camera_vector = rot_matrix.dot(init_camera_vector)
        up_vector = rot_matrix.dot(init_up_vector)
        view_matrix = p.computeViewMatrix(com_p-0.04*camera_vector ,com_p-np.array([0,0,0.1]), up_vector)

        fov, aspect, nearplane, farplane = 60, 1.0, 0.01, 100
        projection_matrix = p.computeProjectionMatrixFOV(fov, aspect, nearplane, farplane)

        (_, _, rgb, depth, mask) = p.getCameraImage(self.camera_width, self.camera_width, view_matrix, projection_matrix)
        rgb = rgb[:, :, :3]
    ...
    def static_camera(self):
        ...
        cam_pitch = -40
        cam_roll = 0
        RENDER_HEIGHT = self.camera_width
        RENDER_WIDTH = self.camera_width
        nearVal = 0.1
        farVal = 100.0

        view_matrix = p.computeViewMatrixFromYawPitchRoll(cameraTargetPosition=[0.5, 0.0, 0.225],
                                                                        distance=cam_dist,
                                                                        yaw=cam_yaw,
                                                                        pitch=cam_pitch,
                                                                        roll=cam_roll,
        ...
        (_, _, rgb, depth, mask) = p.getCameraImage(width=RENDER_WIDTH, height=RENDER_HEIGHT,
                                                                    viewMatrix=view_matrix,
                                                                    projectionMatrix=proj_matrix,)
                                                                    # renderer=p.ER_BULLET_HARDWARE_OPENGL,
                                                                    # physicsClientId=physicsClientId)

        rgb = np.array(rgb, dtype=np.uint8).reshape((RENDER_HEIGHT, RENDER_WIDTH, 4))
        rgb = rgb[:, :, :3]
    ...
    def load_lego(self):
        ...
        x = np.random.uniform(0.4-self.range/2, 0.4+self.range/2)
        y = np.random.uniform(-0.3-self.range/2, -0.3+self.range/2)
        self._obj_init_pose = [x, y, 0.03, 0, 0, 0, 1]
    ...
    def reset_lego(self):
        ...
        y = np.random.uniform(-0.3-self.range/2, -0.3+self.range/2)
        self._obj_init_pose = [x, y, 0.03, 0, 0, 0, 1]
        p.resetBasePositionAndOrientation(self.obj_id, self._obj_init_pose[:3], self._obj_init_pose[3:7],
                                          physicsClientId=self._physics_client_id)
    ...
    def cal_dynamics_param(self):
        """
        Note: Use cal_dynamics_param2() instead!!!
        :return: Mlist, Glist, Slist
        """
        screw_axis = self.get_screw_axis()
        Glist = []
        for index in range(p.getNumJoints(self.robot_id)):
            ...
            joint_info = p.getJointInfo(self.robot_id, index, physicsClientId=self._physics_client_id)
        ...
        Mlist = []
        # Mlist_w2j = self.compute_Mlist([-1, 0, 1, 2, 3, 4, 5, 6, 11])
        Mlist_w2l = self.compute_Mlist(range(-1, 12))
        # baseTrans =
        # Mlist.append(baseTrans)
        for i in range(0, len(Mlist_w2l)):
    ...
    def cal_dynamics_param2(self, eff_link_idx):
        ...
        :return: Mlist, Glist, Slist
        """
        # eff_link_idx：将哪个link设定为最后一个link，eff frame与该link的frame重合，即对应书中link_n
        Glist = []
    ...
    def do_inverse_dynamics(self, th_initial, th_final, final_time=2):
        """
        Function to do inverse dynamics, and perform a feed-forward control.
        Using p.calculateInverseDynamics to compute torque
        ...
        :return: none
        """
        Mlist, Glist, Slist = self.cal_dynamics_param()
        ...
        while n < traj_points:
            ...
            tau1 = inverse_dynamics(list(q_d[n])[0:-2], list(dq_d[n])[0:-2], list(ddq_d[n])[0:-2],
                                    np.array([0, 0, -9.8]), np.array([0, 0, 0, 0, 0, 0]), Mlist, Glist, Slist.T)

            # tau += kd * dq_d[n] #for joint damping applied
            # print(tau)
            ...
            n += 1
        M = self.cal_eff_trans()
        pos, vel, torque = self.getJointStates()
    ...
    def computedTorqueControl(self, th_initial, th_final, final_time=2, controller_gain=400):
        ...
        # forward dynamics simulation loop
        # for turning off link and joint damping
        for link_idx in range(-1, p.getNumJoints(self.robot_id) + 1):
            p.changeDynamics(self.robot_id, link_idx, linearDamping=0.0, angularDamping=0.0, jointDamping=0.0)
            p.changeDynamics(self.robot_id, link_idx, maxJointVelocity=200)
    ...
    def computedTorqueControl2(self, th_initial, th_final, final_time=2, controller_gain=400, grasp=0):
        ...
        while n < q_d.shape[0]:
            ...
            # ------------------------------ #
            """
            实际作业中下一行代码将被注释掉，该行为计算compute torque control公式
            """
            tau = inverse_dynamics(list(q), list(dq), list(aq), [0, 0, -9.8], [0, 0, 0, 0, 0, 0], Mlist, Glist, Slist.T)
    ...
    def joint_PID_control(self, th_initial, th_final, kps, kds, kis, final_time=2):
        ...
        :param th_initial: desired initial joint position
        :param th_final: desired final joint position
        :param kps: list of kp values
        :param kds: list of kd values
        :param kis: list of ki values
        :param final_time: time in seconds
        :return: none
        """
        ...
        while n < q_d.shape[0]:
            ...
            qdes = np.array(q_d[n])
            qdotdes = np.array(dq_d[n])
            qError = qdes - q
            qdotError = qdotdes - qdot

            eint = eint + qError * self.time_step

            Kp = np.diagflat(kps)
            Kd = np.diagflat(kds)
            Ki = np.diagflat(kis)
            forces = Kp.dot(qError) + Kd.dot(qdotError) + Ki.dot(eint)
            self.apply_action(forces)
            p.stepSimulation()
            time.sleep(self.time_step)
            n += 1
